...
So my first idea was to create some terminal script that allowed me to do it. But now that always requires me to first open the terminal just to then open VSCode. So why not expand that into a popup just like I did with the package manager above.

![][/content/posts/popup-tools/vsc-popup.png]

Now I can simply run `Ctrl+T` (tbd, might change) and opens a nice popup that allows me to pick the VSCode workspace I want to resume.
